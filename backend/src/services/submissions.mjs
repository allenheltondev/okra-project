...
export function validateSubmissionPayload(payload) {
  ...
  if (payload.contributorEmail && typeof payload.contributorEmail !== 'string') {
    issues.push('contributorEmail must be a string when provided');
  }

  if (payload.contributorName && typeof payload.contributorName !== 'string') {
    issues.push('contributorName must be a string when provided');
  }

  if (payload.storyText && typeof payload.storyText !== 'string') {
    issues.push('storyText must be a string when provided');
  }
