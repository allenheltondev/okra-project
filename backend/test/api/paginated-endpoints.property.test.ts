...
/** Generate a submission with random status, coordinates, and timestamp */
const arbSubmission = fc.record({
  ...
  display_lat: fc.oneof(
    ...
    fc.constant(0) // include null-island candidates
  ),
...
// ═══════════════════════════════════════════════════════════════════════════
// Property 1: Public Endpoint Pagination Completeness
// ═══════════════════════════════════════════════════════════════════════════

// Feature: paginated-endpoints, Property 1: Public Endpoint Pagination Completeness
describe('Property 1: Public Endpoint Pagination Completeness', () => {
  // **Validates: Requirements 1.1, 1.4, 1.5, 7.7, 7.8, 7.9, 8.1**
  it('iterating all pages yields every approved non-null-island submission exactly once in created_at DESC order', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(arbSubmission, { minLength: 0, maxLength: 30 }),
        fc.integer({ min: 1, max: 10 }),
        async (submissions, pageSize) => {
          // Determine the expected set: approved AND NOT null-island
          const isNullIsland = (s: any) => s.display_lat === 0 && s.display_lng === 0;
          const expected = submissions
            .filter((s) => s.status === 'approved' && !isNullIsland(s))
            .sort((a, b) => {
              const timeDiff = b.created_at.getTime() - a.created_at.getTime();
              if (timeDiff !== 0) return timeDiff;
              return b.id.localeCompare(a.id); // DESC by id as tiebreaker
            });

          // Set up mock to simulate the handler's SQL queries with cursor-based pagination
          queryResponses = {
            'FROM submissions': (_text: string, params: any[]) => {
              const limit = params[params.length - 1]; // last param is always limit

              // Filter to approved, non-null-island
              ...
              if (params.length === 3) {
                ...
                filtered = filtered.filter((s) => {
                  ...
                  if (timeDiff < 0) return true; // earlier created_at → comes after in DESC
                  if (timeDiff === 0) return s.id < cursorId; // same time, smaller id → after in DESC
                  return false;
                });
              }
          ...
          do {
            ...
            const res = await handler(
              makeRestApiEvent('/okra', 'GET', { queryStringParameters: qsp })
            );
          ...
          // 2. Every expected submission appears exactly once (by id)
          ...
          // 4. Results are in created_at DESC order
...
// ═══════════════════════════════════════════════════════════════════════════
// Property 2: Public Endpoint Response Shape
// ═══════════════════════════════════════════════════════════════════════════

// Feature: paginated-endpoints, Property 2: Public Endpoint Response Shape
describe('Property 2: Public Endpoint Response Shape', () => {
  // **Validates: Requirements 1.2, 1.3**
  it('each item has required fields and excludes sensitive fields', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.array(arbSubmission.filter((s) => s.status === 'approved' && !(s.display_lat === 0 && s.display_lng === 0)), {
          minLength: 1,
          maxLength: 20,
        }),
        async (approvedSubmissions) => {
          // Set up mock to return these approved, non-null-island submissions
          queryResponses = {
            'FROM submissions': (_text: string, _params: any[]) => {
              const rows = approvedSubmissions
                .sort((a, b) => {
                  const timeDiff = b.created_at.getTime() - a.created_at.getTime();
                  if (timeDiff !== 0) return timeDiff;
                  return b.id.localeCompare(a.id);
                ...
                .map((s) => ({
                  ...
                  created_at: s.created_at,
                  created_at_raw: s.created_at.toISOString().replace('T', ' ').replace('Z', '+00'),
                  // Include sensitive fields in DB rows to verify they are stripped
                  contributor_email: `user-${s.id.slice(0, 4)}@example.com`,
                  raw_location_text: '123 Main St',
                  geocode_lat: s.display_lat + 0.001,
                  geocode_lng: s.display_lng + 0.001,
...
// ═══════════════════════════════════════════════════════════════════════════
// Property 3: Public Endpoint Photo URLs Are CloudFront
// ═══════════════════════════════════════════════════════════════════════════

// Feature: paginated-endpoints, Property 3: Public Endpoint Photo URLs Are CloudFront
describe('Property 3: Public Endpoint Photo URLs Are CloudFront', () => {
  // **Validates: Requirements 2.1, 2.2, 2.3**

  /** Generate a random S3 key for a thumbnail */
  const arbS3Key = fc
    .tuple(fc.uuid(), fc.uuid(), fc.constantFrom('thumbnail.webp', 'thumb.webp', 'photo.webp'))
    .map(([subId, photoId, filename]) => `submissions/${subId}/${photoId}/${filename}`);

  /** Generate an approved, non-null-island submission with optional photo keys */
  const arbApprovedSubmissionWithPhotos = fc.record({
    id: fc.uuid(),
    privacy_mode: arbPrivacyMode,
    display_lat: fc.double({ min: -90, max: 90, noNaN: true, noDefaultInfinity: true }).filter((v) => v !== 0),
    display_lng: fc.double({ min: -180, max: 180, noNaN: true, noDefaultInfinity: true }).filter((v) => v !== 0),
    created_at: fc.date({ min: new Date('2020-01-01'), max: new Date('2030-01-01') }),
    photoKeys: fc.array(arbS3Key, { minLength: 0, maxLength: 5 }),
  });

  const S3_SIGNING_PARAMS = ['X-Amz-Signature', 'X-Amz-Credential', 'X-Amz-Date', 'X-Amz-Expires', 'X-Amz-SignedHeaders'];
  const CDN_PREFIX = 'https://dtest123.cloudfront.net/';

  it('every photo URL is a CloudFront URL with no S3 signing params; empty array when no ready photos', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(arbApprovedSubmissionWithPhotos, { minLength: 1, maxLength: 15 }),
        async (submissions) => {
          // Build photo rows keyed by submission_id
          const allPhotoRows: Array<{ submission_id: string; thumbnail_s3_key: string }> = [];
          for (const s of submissions) {
            for (const key of s.photoKeys) {
              allPhotoRows.push({ submission_id: s.id, thumbnail_s3_key: key });
            }
          }
          ...
          for (const item of body.data) {
            ...
            // Find the original submission to check expected photo count
            const original = submissions.find((s) => s.id === item.id);
            if (!original) continue;

            if (original.photoKeys.length === 0) {
              // When a submission has no ready photos, photo_urls should be empty
              expect(item.photo_urls).toEqual([]);
            } else {
              expect(item.photo_urls.length).toBe(original.photoKeys.length);
            }

            // Every URL must start with the CloudFront CDN prefix
            for (const url of item.photo_urls) {
              expect(url.startsWith(CDN_PREFIX)).toBe(true);
            }

            // No URL should contain S3 signing parameters
            for (const url of item.photo_urls) {
              for (const param of S3_SIGNING_PARAMS) {
...
// ═══════════════════════════════════════════════════════════════════════════
// Property 4: Public Endpoint Photo Completeness and Ordering
// ═══════════════════════════════════════════════════════════════════════════

// Feature: paginated-endpoints, Property 4: Public Endpoint Photo Completeness and Ordering
describe('Property 4: Public Endpoint Photo Completeness and Ordering', () => {
  ...
  /** Generate a photo with a created_at timestamp and S3 key */
  const arbPhoto = fc.record({
    created_at: fc.date({ min: new Date('2020-01-01'), max: new Date('2030-01-01') }),
    thumbnail_s3_key: arbS3Key,
    status: fc.constantFrom('ready', 'uploaded', 'processing', 'failed') as fc.Arbitrary<'ready' | 'uploaded' | 'processing' | 'failed'>,
  });

  /** Generate an approved, non-null-island submission with photos that have mixed statuses */
  const arbApprovedSubmissionWithTimedPhotos = fc.record({
    id: fc.uuid(),
    privacy_mode: arbPrivacyMode,
    display_lat: fc.double({ min: -90, max: 90, noNaN: true, noDefaultInfinity: true }).filter((v) => v !== 0),
    display_lng: fc.double({ min: -180, max: 180, noNaN: true, noDefaultInfinity: true }).filter((v) => v !== 0),
    created_at: fc.date({ min: new Date('2020-01-01'), max: new Date('2030-01-01') }),
    photos: fc.array(arbPhoto, { minLength: 0, maxLength: 6 }),
  });

  const CDN_DOMAIN = 'dtest123.cloudfront.net';

  it('photo_urls contains exactly the ready photos ordered by created_at ASC', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(arbApprovedSubmissionWithTimedPhotos, { minLength: 1, maxLength: 10 }),
        async (submissions) => {
          // Compute expected ready photos per submission, sorted by created_at ASC
          const expectedPhotosPerSubmission = new Map<string, Array<{ created_at: Date; thumbnail_s3_key: string }>>();
          for (const s of submissions) {
          ...
          // Build photo rows as the real query would return them:
          // sorted by submission_id, created_at ASC, only ready photos
          const allPhotoRows: Array<{ submission_id: string; thumbnail_s3_key: string; created_at: Date }> = [];
          for (const s of submissions) {
            const readyPhotos = s.photos
              .filter((p) => p.status === 'ready')
              .sort((a, b) => a.created_at.getTime() - b.created_at.getTime());
          ...
          queryResponses = {
            ...
            'FROM submission_photos': (_text: string, params: any[]) => {
              const requestedIds: string[] = params[0];
              // Return only ready photos for requested submissions, sorted by submission_id, created_at ASC
              const rows = allPhotoRows
                .filter((p) => requestedIds.includes(p.submission_id))
                .sort((a, b) => {
          ...
          for (const item of body.data) {
            ...
            const expectedUrls = expectedReady.map(
              (p) => `https://${CDN_DOMAIN}/${p.thumbnail_s3_key}`
            );

            // Completeness: photo_urls contains exactly the ready photos
            expect(item.photo_urls.length).toBe(expectedUrls.length);

            // Ordering: photo_urls are in created_at ASC order (matching expected)
            expect(item.photo_urls).toEqual(expectedUrls);
          }
        }
      ),
      { numRuns: 100 }
...
// ═══════════════════════════════════════════════════════════════════════════
// Property 8: Limit Parameter Validation
// ═══════════════════════════════════════════════════════════════════════════

// Feature: paginated-endpoints, Property 8: Limit Parameter Validation
describe('Property 8: Limit Parameter Validation', () => {
  // **Validates: Requirements 7.4, 7.5**

  /** Generator for invalid limit strings: non-numeric, decimals, zero, negative */
  const arbInvalidLimit = fc.oneof(
    ...
    // Decimal numbers like "3.7", "1.5", "0.1"
    fc.tuple(
  ...
  /** Generator for valid limit values (1–100) */
  const arbValidLimit = fc.integer({ min: 1, max: 100 });

  /** Generator for limits above 100 (should be clamped) */
  const arbOverLimit = fc.integer({ min: 101, max: 10000 });

  it('invalid limits (non-numeric, decimal, zero, negative) return 400 INVALID_LIMIT on GET /okra', async () => {
    await fc.assert(
      fc.asyncProperty(arbInvalidLimit, async (invalidLimit) => {
        resetMocks();
        queryResponses = {};

        const res = await handler(
          makeRestApiEvent('/okra', 'GET', {
            queryStringParameters: { limit: invalidLimit },
          })
        );
        const { statusCode, body } = parseRes(res);
  ...
  it('valid limits (1–100) return at most `limit` items on GET /okra', async () => {
    await fc.assert(
      fc.asyncProperty(
        arbValidLimit,
        fc.array(arbSubmission.filter((s) => s.status === 'approved' && !(s.display_lat === 0 && s.display_lng === 0)), {
        ...
        async (limit, submissions) => {
          queryResponses = {
            'FROM submissions': (_text: string, params: any[]) => {
              const queryLimit = params[params.length - 1];
              const rows = submissions
                .sort((a, b) => {
                  const timeDiff = b.created_at.getTime() - a.created_at.getTime();
                  if (timeDiff !== 0) return timeDiff;
  ...
  it('limits above 100 are clamped to 100 (at most 100 items) on GET /okra', async () => {
    await fc.assert(
      fc.asyncProperty(arbOverLimit, async (overLimit) => {
        // Generate exactly 120 approved submissions so there are more than 100 available
        const submissions = Array.from({ length: 120 }, (_, i) => ({
          id: `${String(i).padStart(8, '0')}-0000-0000-0000-000000000000`,
          contributor_name: `User-${i}`,
          story_text: `Story ${i}`,
          privacy_mode: 'exact',
          display_lat: 10 + i * 0.01,
          display_lng: 20 + i * 0.01,
          created_at: new Date(2025, 0, 1, 0, 0, i),
          created_at_raw: new Date(2025, 0, 1, 0, 0, i).toISOString().replace('T', ' ').replace('Z', '+00'),
        }));

        queryResponses = {
          'FROM submissions': (_text: string, params: any[]) => {
            const queryLimit = params[params.length - 1];
            // Return up to queryLimit rows (handler requests limit+1)
            const rows = submissions.slice(0, queryLimit);
            return { rows, rowCount: rows.length };
          },
          'FROM submission_photos': { rows: [], rowCount: 0 },
        };
        ...
        const res = await handler(
          makeRestApiEvent('/okra', 'GET', {
            queryStringParameters: { limit: String(overLimit) },
          })
        );
        const { statusCode, body } = parseRes(res);
        expect(statusCode).toBe(200);
        // Clamped to 100: data should have at most 100 items
...
describe('Property 9: Invalid Cursor Rejection', () => {
  ...
  it('invalid cursor values return 400 INVALID_CURSOR on GET /okra', async () => {
    await fc.assert(
...
describe('Property 13: Error Response Shape Consistency', () => {
  ...
  it('error responses from GET /okra have shape { error: { code, message } } with non-empty strings', async () => {
    await fc.assert(
      fc.asyncProperty(arbErrorTrigger, async ({ queryStringParameters, expectedCode }) => {
        ...
        const res = await handler(
          makeRestApiEvent('/okra', 'GET', { queryStringParameters })
        );
        const { statusCode, body } = parseRes(res);
  ...
  it('database error responses (500) have shape { error: { code, message } } with non-empty strings on GET /okra', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constant(null), // no varying input needed; just run multiple times for confidence
        async () => {
          // Force a database error by making query throw (inside the handler's try/catch)
          resetMocks();
          queryResponses = {};
          ...
          const res = await handler(
            makeRestApiEvent('/okra', 'GET', { queryStringParameters: null })
          );
          const { statusCode, body } = parseRes(res);
